...
ARQUIVO_CACHE = "ultima_atualizacao.txt"
...
def verificar_se_ja_atualizou_hoje():
    """Verifica se o arquivo de cache possui a data do dia atual."""
...
def salvar_registro_de_atualizacao():
    """Salva a data de hoje para bloquear novas execuções repetidas."""
    data_hoje = datetime.now().strftime("%Y-%m-%d")
...
# 🔒 TRAVA DIÁRIA: Bloqueia a execução se já tiver rodado hoje
if verificar_se_ja_atualizou_hoje():
    print(f"⏸️ O sistema já foi atualizado hoje ({datetime.now().strftime('%d/%m/%Y')}).")
    print("Nenhuma requisição foi enviada à API para poupar recursos do seu servidor.")
...
for loteria_local, loteria_api in MAPA_LOTERIAS.items():
    try:
        ...
        if resposta.status_code == 200:
            ...
            dezenas_lista = [int(x) for x in dados.get("dezenas", [])]
...
# Grava a trava diária se as requisições principais rodarem sem falhas de conexão
if not erros_detectados:
    salvar_registro_de_atualizacao()
    print("\n💾 Data de atualização salva. Nova verificação liberada apenas amanhã.")
